...
const LoginPage: React.FC<LoginPageProps> = ({ users, onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl grid lg:grid-cols-2 gap-8 items-start">
        {/* Login Form Card */}
        <div className={`bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl p-8 ${isShaking ? 'animate-shake' : ''}`}>
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">BRD Architect</h1>
            <p className="text-slate-500 text-sm mt-1">AI-Powered Business Requirements</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border-2 border-slate-200 rounded-xl focus:border-indigo-500 focus:bg-white outline-none transition-all text-sm font-medium"
            ...
            <button
              type="submit"
              className="w-full py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all shadow-lg shadow-indigo-200 hover:shadow-xl hover:shadow-indigo-300 active:scale-[0.98]"
            >
              Sign In
            </button>
          </form>
          ...
          <div className="mt-6 pt-6 border-t border-slate-200 text-center">
            <p className="text-xs text-slate-400 font-medium">
              DEMO VERSION • Click any credential card to auto-fill →
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="text-white mb-2">
            ...
            <p className="text-white/60 text-sm">Click to auto-fill login form</p>
          </div>
          ...
          {DEMO_CREDENTIALS.map((cred, idx) => (
            <button
              ...
              className="w-full text-left bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-4 hover:bg-white/20 transition-all group hover:scale-[1.02] active:scale-[0.98]"
              style={{ animationDelay: `${idx * 100}ms` }}
            >
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${getRoleColor(cred.color)} shadow-lg flex items-center justify-center text-white font-black text-lg`}>
                  {cred.role.charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-bold text-white">{cred.role}</span>
                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${getRoleBadgeColor(cred.color)}`}>
                      {cred.role === 'Admin' ? 'FULL ACCESS' : 'LIMITED'}
                    </span>
                  </div>
                  <p className="text-white/60 text-xs truncate">{cred.description}</p>
                  <div className="flex items-center gap-4 mt-2">
                    <code className="text-[10px] bg-black/20 px-2 py-1 rounded text-white/80 font-mono">{cred.email}</code>
                    <code className="text-[10px] bg-black/20 px-2 py-1 rounded text-white/80 font-mono">{cred.password}</code>
                  </div>
                </div>
          ...
          <div className="bg-amber-500/20 border border-amber-500/30 rounded-xl p-4 mt-4">
            <div className="flex items-start gap-3">
              <svg className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
              </svg>
              <div>
                <p className="text-amber-200 text-xs font-bold mb-1">Prototype Mode</p>
                <p className="text-amber-200/70 text-[11px] leading-relaxed">
                  This is a demo version with hardcoded credentials. In production, integrate with your enterprise SSO/OAuth provider.
                </p>
              </div>
            </div>
          </div>
