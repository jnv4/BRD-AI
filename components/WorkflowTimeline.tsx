  ...
  return (
    <div className="relative flex justify-between overflow-x-auto pb-2">
      ...
      {steps.map((step, idx) => {
        ...
        return (
          <div key={step.key} className="relative z-10 flex flex-col items-center flex-1 min-w-[80px]">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-500 ${
              isCompleted 
                ...
                : isActive 
                  ? isVerificationStep
                    ? step.key === BRDStatus.VERIFIED
                      ? 'bg-white border-4 border-teal-600 text-teal-600 shadow-lg'
                      : 'bg-white border-4 border-purple-600 text-purple-600 shadow-lg'
                    : step.key === BRDStatus.APPROVED
                      ? 'bg-white border-4 border-emerald-600 text-emerald-600 shadow-lg'
